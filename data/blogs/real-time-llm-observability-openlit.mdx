...
Start by inventorying every surface that touches an LLM model, from cron jobs to API handlers. The Python SDK example in the configuration guide shows the minimal init block:
...
The TypeScript SDK mirrors the same parameters, so your edge routes remain consistent:
...
Instrument these entry points first so every downstream step has trustworthy source data.
Once traces are flowing locally, run your usual load tests and confirm that prompts,
responses, token counts, and latency histograms appear in your OTLP destination. Any
gaps you detect here will ripple through the rest of the observability stack.
...
- **Unified SDK surface** means Python, Node.js, and operator-managed workloads emit identical attributes, so cross-team investigations move faster.
